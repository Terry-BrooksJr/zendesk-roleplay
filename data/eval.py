...
def write_reports(cfg, results):
    ...
    for r in json_report["results"]:
        ...
        md_lines.append(f"- Accuracy: **{r['accuracy']:.3f}**")
        md_lines.append(f"- F1 (macro): **{r['f1_macro']:.3f}**")
        md_lines.append(f"- Latency p50: **{r['latency_p50']*1000:.1f} ms**")
        md_lines.append(f"- Latency p95: **{r['latency_p95']*1000:.1f} ms**")
        # thresholds
        checks = r["thresholds"]["checks"]
        if checks:
        ...
        md_lines.append(df_cm.to_markdown())
        md_lines.append("\n---\n")
